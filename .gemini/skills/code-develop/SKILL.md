...
- Confirm the intended principle or pattern against its official definition (`web-search`, `fetch-markdown`; search fallback per the search strategy in `GEMINI.md`) so usage matches purpose
...
- Run the project's tests and linter with the language's toolchain, per the execution rules in `GEMINI.md` (e.g. Python via `uv run`, Node via `npx`), to confirm behavior is preserved
