...
- Follow the stack's official layout and toolchain, per the execution rules in `GEMINI.md` (e.g. Python `uv` workspace with `hatchling` and `src/<package>/` via `uv`, Node `package.json` with `npx` tooling)
