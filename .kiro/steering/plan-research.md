...
Research synthesis workflow that runs after `plan-search` to triage results, fetch canonical sources, reason with sequential-thinking, and ground claims. Use when investigating specs, APIs, or standards before writing or editing documentation.

Research runs after `plan-search`, where search gathers and research reasons. The phases are worked in order, and a loop back is made when one exposes a gap.
